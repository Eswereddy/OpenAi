...
// database (db/index.js). Started as five endpoints and has grown with each
// new feature since; no auth anywhere because no personal or sensitive data
// is ever collected.
...
// GET /api/schemes/:id/reminder-plan — NEW FEATURE: exposes the same
// per-scheme follow-up horizon reminder.js already computes for the .ics
// file (see buildReminderIcs above), but as plain JSON with no file
// generated. Powers the client-side "priority roadmap" timeline, which
// needs to know *when* to bucket a scheme ("this month" vs "in a few
// months") without downloading a calendar file for every matched scheme
// just to read a number out of it. Purely additive — reminder.js's own
// planFor() already existed and is unchanged, this just gives the frontend
// a second, cheaper way to read it.
app.get("/api/schemes/:id/reminder-plan", reminderLimiter, (req, res) => {
  try {
    const schemeId = String(req.params.id || "").slice(0, 64);
    const plan = planFor(schemeId);
    res.json({ schemeId, days: plan.days, note: plan.note });
  } catch (err) {
    console.error("GET /api/schemes/:id/reminder-plan failed:", err);
    res.status(500).json({ error: "Could not load a reminder horizon right now." });
  }
});
...
// POST /api/schemes/reminder-plans — NEW FEATURE: batch version of GET
// /api/schemes/:id/reminder-plan above. The priority-roadmap timeline needs
// a follow-up horizon for every one of a citizen's matched schemes at once
// (could be a dozen); looking each one up with its own round trip works but
// wastes N-1 of them on pure network overhead for what is, underneath,
// the same in-memory planFor() lookup repeated N times. One request in,
// one array out — same data, same rules, fewer round trips.
app.post("/api/schemes/reminder-plans", reminderLimiter, (req, res) => {
  try {
    const ids = Array.isArray((req.body || {}).ids) ? req.body.ids : [];
    const plans = ids
      .filter((id) => typeof id === "string" && VALID_SCHEME_IDS.has(id))
      .slice(0, 25)
      .map((id) => {
        const plan = planFor(id);
        return { schemeId: id, days: plan.days, note: plan.note };
      });
    res.json({ plans });
  } catch (err) {
    console.error("POST /api/schemes/reminder-plans failed:", err);
    res.status(500).json({ error: "Could not load reminder horizons right now." });
  }
});

// POST /api/schemes/reminders/bulk.ics — NEW FEATURE: "Remind me about all
// of these" — a single downloadable calendar with one event per matched
// scheme the citizen chooses to keep following up on, instead of the
// single-scheme reminder above repeated once per scheme. POST (not GET)
// because the payload is a list of schemes rather than one id, which could
// exceed a comfortable URL length once a citizen has several matches.
// Same no-server-storage, no-eligibility-decision boundary as the
// single-scheme route: schemeName/statusLabel are display text only, taken
// from the client's own already-computed matches.
app.post("/api/schemes/reminders/bulk.ics", reminderLimiter, (req, res) => {
  try {
    const rawItems = Array.isArray((req.body || {}).items) ? req.body.items : [];
    const items = rawItems
      .filter((it) => it && typeof it.schemeId === "string" && VALID_SCHEME_IDS.has(it.schemeId))
      .slice(0, 25)
      .map((it) => ({
        schemeId: it.schemeId,
        schemeName: typeof it.schemeName === "string" ? it.schemeName.slice(0, 200) : it.schemeId,
        statusLabel: typeof it.statusLabel === "string" ? it.statusLabel.slice(0, 40) : null,
      }));
    if (!items.length) {
      return res.status(400).json({ error: "At least one valid scheme is required." });
    }
    const ics = buildBulkReminderIcs(items);
    res.setHeader("Content-Type", "text/calendar; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename="am-i-eligible-reminders.ics"`);
    res.send(ics);
  } catch (err) {
    console.error("POST /api/schemes/reminders/bulk.ics failed:", err);
    res.status(500).json({ error: "Could not build reminders right now." });
  }
});
...
app.post("/api/feedback", feedbackLimiter, (req, res) => {
  try {
    ...
    feedbackStatsCache = null; // invalidate so the next reader sees this vote, not a stale cache
    res.json({ ok: true });
  } catch (err) {
...
// Small in-memory cache for GET /api/feedback/stats, in the same spirit as
// impact-stats.js's own 15s cache. This endpoint used to be dashboard-only
// (polled occasionally by a human); it's now also fetched once per citizen
// every time results render (see public/index.html's per-card feedback
// badge), so a full GROUP BY table scan on every single results page load
// is wasted work when the underlying numbers only change a few times a
// minute at most. A new vote still lands immediately (logFeedback below is
// untouched); this only invalidates the cache right after so the next
// reader sees it, rather than waiting out the TTL.
let feedbackStatsCache = null;
let feedbackStatsCacheAt = 0;
const FEEDBACK_STATS_CACHE_MS = 10_000;

function getFeedbackStatsCached() {
  const now = Date.now();
  if (feedbackStatsCache && now - feedbackStatsCacheAt < FEEDBACK_STATS_CACHE_MS) return feedbackStatsCache;
  feedbackStatsCache = getFeedbackStats();
  feedbackStatsCacheAt = now;
  return feedbackStatsCache;
}

// GET /api/feedback/stats — NEW FEATURE: aggregated, per-scheme feedback
// numbers for the analytics dashboard (public/dashboard.html) and for the
// citizen-facing "X% found this helpful" badge on each scheme card. No raw
// comments returned here on purpose — this is a lightweight numbers view,
// not a place to surface free text without a moderation step.
app.get("/api/feedback/stats", (req, res) => {
  try {
    res.json({ feedback: getFeedbackStatsCached() });
  } catch (err) {
